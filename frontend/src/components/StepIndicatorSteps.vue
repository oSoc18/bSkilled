<template>
  ...
  <ul class="progressbar"  v-else="currentProcess === 'signing' || currentFlowStep === 'sign'">
    <li class="progressbar_item" v-bind:class="{'current': currentPageCheck('sign'), 'visited': visitedPageCheckSigning('sign'), 'visited-animation': checkVisitedSearch(pageVisited)}"><a class="progressbar_item_link" v-bind:class="{'link--disabled': !visitedPageCheckSigning('sign')}" href="#" @click="$router.go(-1)">Seems like you want to sign</a></li>
