<template>
  ...
  <ul class="progressbar"  v-else="currentProcess === 'signing' || currentFlowStep === 'sign'">
    <li class="progressbar_item" v-bind:class="{'current': currentPageCheck('sign'), 'visited': visitedPageCheckSigning('sign'), 'visited-animation': checkVisitedSearch(pageVisited)}"><a class="progressbar_item_link" v-bind:class="{'link--disabled': !visitedPageCheckSigning('sign')}" href="#" @click="$router.go(-1)">{{$t("StepIndicatorStepsSigning.sign")}}</a></li>
    <li class="progressbar_item" v-bind:class="{'current': currentPageCheck('upload'), 'visited': visitedPageCheckSigning('upload'), 'visited-animation': checkVisitedInformation(pageVisited), 'visited-delay': checkVisitedSearch(pageVisited)}"><a class="progressbar_item_link" v-bind:class="{'link--disabled': !visitedPageCheckSigning('upload')}"  href="#" @click="$router.go(-1)">{{$t("StepIndicatorStepsSigning.upload")}}</a></li>
    <li class="progressbar_item" v-bind:class="{'current': currentPageCheck('profile'), 'visited': visitedPageCheckSigning('profile'),'visited-animation': checkVisitedprofile(pageVisited), 'visited-delay': checkVisitedInformation(pageVisited)}"><a class="progressbar_item_link" v-bind:class="{'link--disabled': !visitedPageCheckSigning('profile')}" href="#"  @click="$router.go(-1)">{{$t("StepIndicatorStepsSigning.profile")}}</a></li>
    <li class="progressbar_item" v-bind:class="{'current': currentPageCheck('confirm'), 'visited': visitedPageCheckSigning('confirm')}"><a class="progressbar_item_link" v-bind:class="{'link--disabled': !visitedPageCheckSigning('confirm')}" href="#">{{$t("StepIndicatorStepsSigning.confirm")}}</a></li>
  </ul>
</template>
